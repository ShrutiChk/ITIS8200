...
class SecurePRNG:
    ...
    def generate(self, n_bytes):
        ...
        while len(output) < n_bytes:
            ...
            block = hashlib.sha256(str(self.state).encode()).digest()
            output += block
            # 2. Update state immediately after with a hash function (One-way progression)
            self.state = hashlib.sha256(str(self.state).encode()).digest()
...
def xor_crypt(data, prng):
    ...
    return bytes(data ^ key for data, key in zip(data, keystream))
...
class Mallory:
    ...
    def intercept(self, sender, recipient, payload):
        ...
        if isinstance(payload, bytes):
            ...
            print(f"[MALLORY] Intercepted Plaintext from {sender}: {plaintext.decode()}")
...
def main():
    ...
    message = b"Use Abanti for the test" # Put in your test message here
